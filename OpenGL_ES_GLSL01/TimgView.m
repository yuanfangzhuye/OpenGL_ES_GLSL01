...
- (void)setupLayer
{
    //1.创建特殊图层
    /*
     重写 layerClass，将 TimgView 返回的图层从 CALayer 替换成 CAEAGLLayer
     */
    ...
    /**
     ...
     kEAGLDrawablePropertyColorFormat 可绘制表面的内部颜色缓存区格式，这个 key 对应的值是一个 NSString 指定特定颜色缓存区对象。默认是 kEAGLColorFormatRGBA8；
...
- (void)setupContext
{
    //1.创建图形上下文，并指定OpenGL ES 渲染API版本，我们使用2.0
...
- (void)setupFrameBuffer
{
    ...
    //5.将渲染缓存区 myColorRenderBuffer 通过 glFramebufferRenderbuffer 函数绑定到 GL_COLOR_ATTACHMENT0上
...
- (void)setupVertDatas
{
    ...
    //4.----处理纹理数据-------
    //(1).glGetAttribLocation,用来获取vertex attribute的入口的.
    //⚠️第二参数字符串必须和shaderv.vsh中的输入变量：textCoordinate保持一致
...
{
    //1. 设置纹理采样器 sampler2D
    GLuint sampler = glGetUniformLocation(self.myPrograme, "colorMap");
